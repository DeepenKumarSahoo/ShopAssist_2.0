...
def compare_laptops_with_user(user_requirements):
    ...
    # Fetch real image URLs using Google Custom Search API
    top_laptops['Image_URL'] = top_laptops.apply(
        lambda row: get_laptop_image_url(row['Brand'], row['Model Name']),
        axis=1
    )
...
def get_laptop_image_url(brand, model):
    """
    ...
    # You'll need to replace these with your own API key and CSE ID
    GOOGLE_API_KEY = "#####"
    GOOGLE_CSE_ID = "########"
    
    # Fallback URL in case the API call fails
    fallback_url = f"https://via.placeholder.com/200x200?text={brand}+{model}"
    
    try:
        # Construct the search query
        query = f"{brand} {model} laptop official"
        ...
        print(f"Error fetching image for {brand} {model}: {str(e)}")
        return fallback_url
    except Exception as e:
        print(f"Unexpected error for {brand} {model}: {str(e)}")
        return fallback_url